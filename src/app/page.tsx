...
export default function Home() {
  const { products, isReady, addProduct, editProduct, removeProduct, searchProducts } = useProducts();
  ...
  return <main className="min-h-screen bg-stone-50 pb-10 text-slate-900">
    <div className="mx-auto max-w-2xl px-4 pt-5 sm:px-6">
      ...
      {!isReady ? <div className="mt-8 rounded-2xl bg-white p-6 text-center text-slate-500">Carregando produtos...</div> : view === "search" ? <section className="mt-6">
        ...
        <div className="flex items-center justify-between gap-4"><div><h2 className="text-2xl font-black">Cadastro de produtos</h2><p className="mt-1 text-slate-600">{products.length} produtos cadastrados neste aparelho.</p></div>{!isFormOpen && <button onClick={() => showForm()} className="min-h-14 shrink-0 rounded-xl bg-rose-700 px-4 text-lg font-black text-white hover:bg-rose-800"><Plus className="mr-1 inline size-5" />Novo</button>}</div>
        {isFormOpen ? <ProductForm key={editingProduct?.id ?? "new"} product={editingProduct} onSave={(input) => { if (editingProduct) editProduct(editingProduct.id, input); else addProduct(input); closeForm(); }} onCancel={closeForm} onDelete={editingProduct ? () => { if (window.confirm(`Excluir ${editingProduct.name}?`)) { removeProduct(editingProduct.id); closeForm(); } } : undefined} /> : <ProductList products={products} onEdit={showForm} />}
      </section>}
    </div>
  </main>;
}
